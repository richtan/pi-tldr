...
/** Writes pi-tldr model settings below a test root. */
export function writeModelSettings(
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ "pi-tldr": { model } }, null, 2)}\n`,
  );
}

/** Creates a temporary cwd containing project-local pi-tldr model settings. */
